...
func TestRegisterRequest_Invalid_Request_Body(t *testing.T) {
	config.Load()
	...
	h.POST("/register", regnHandler.HandleRegistration)
	w := ut.PerformRequest(h.Engine, http.MethodPost, "/register", nil)
	resp := w.Result()
	assert.DeepEqual(t, 400, resp.StatusCode())
	assert.DeepEqual(t, "{\"error\":\"JSON body seems to be malformed\"}", string(resp.Body()))
}

func TestRegisterRequest_Missing_Request_Body_Field(t *testing.T) {
	config.Load()
	...
	h.POST("/register", regnHandler.HandleRegistration)
	data := bytes.NewBufferString("{\"name\":\"mayank\", \"role\":\"user\", \"password\":\"hellothere\"}")
	w := ut.PerformRequest(h.Engine, http.MethodPost, "/register",
		&ut.Body{Body: data, Len: data.Len()},
		ut.Header{Key: "Content-Type", Value: "application/json"})
	resp := w.Result()
